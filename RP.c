...
void process_uart() { /* ... UART processing logic remains the same ... */ }
...
uint8_t dpad_to_switch_hat(uint8_t dpad_mask) {
    static const uint8_t hat_map[16] = { 8, 0, 2, 1, 4, 0, 3, 0, 6, 7, 5, 0, 0, 0, 0, 0 }; // SWITCH_HAT_NOTHING is 8
...
void hid_task(void) {
  ...
  if ( tud_hid_ready() ) {
      ...
      // Map buttons (this requires a specific mapping for Switch)
    ...
    #elif INPUT_MODE_DS4
      // TODO: Implement DS4 report mapping
      ...
      // Note: Generic descriptor doesn't have a standard hat, it's buttons
