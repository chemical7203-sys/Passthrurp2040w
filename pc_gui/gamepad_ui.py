...
class GamepadSignals(QObject):
    ...
    # dpad_event is no longer needed as D-pad is now buttons
    gamepad_disconnected = pyqtSignal()
    raw_event = pyqtSignal(str)
    uart_rx = pyqtSignal(str) # For UART RX data
...
class GamepadWidget(QGraphicsView):
    ...
    def _draw_layout(self):
        ...
        self.l3_btn = self._create_stick_item(160, 130) # Visual for L3 click
        self.l3_btn.setZValue(-1) # Draw behind stick
        ...
        self.r3_btn = self._create_stick_item(290, 130) # Visual for R3 click
        self.r3_btn.setZValue(-1)

        # Buttons and D-Pad
        self.buttons = {
            'BTN_WEST': self._create_button_item(350, 95),
            'BTN_NORTH': self._create_button_item(380, 65),
            'BTN_EAST': self._create_button_item(410, 95),
            'BTN_SOUTH': self._create_button_item(380, 125),
            'DPAD_UP': self._create_dpad_item(55, 60, 20, 25),
            'DPAD_DOWN': self._create_dpad_item(55, 115, 20, 25),
            'DPAD_LEFT': self._create_dpad_item(30, 85, 25, 20),
            'DPAD_RIGHT': self._create_dpad_item(75, 85, 25, 20),
            'BTN_TL': self._create_dpad_item(30, 25, 80, 20), # L1
            'BTN_TR': self._create_dpad_item(340, 25, 80, 20), # R1
            'BTN_THUMBL': self.l3_btn,
            'BTN_THUMBR': self.r3_btn,
        }
    ...
    def _create_button_item(self, x, y):
        button = QGraphicsEllipseItem(-10, -10, 20, 20); button.setBrush(self.btn_off_brush); button.setPos(x, y); self.scene.addItem(button)
        return button
