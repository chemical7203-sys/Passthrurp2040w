    ...
    def _refresh_devices(self):
        ...
        for i in range(count):
            try:
                ...
                # Filter out the RP2040 device itself, which appears as "Pro Controller"
                if "Pro Controller" in joystick.get_name():
                    print(f"DEBUG: DS4Handler: Ignoring joystick {i} ({joystick.get_name()}) as it appears to be the output device.")
                    joystick.quit() # Release the ignored joystick
                    continue

                valid_joysticks.append(joystick)
                gamepads_info.append({'name': joystick.get_name(), 'index': len(valid_joysticks) - 1})
            except pygame.error as e:
                print(f"DEBUG: DS4Handler: Could not get info for joystick {i}: {e}")
                continue

        self.joysticks = valid_joysticks
        print(f"DEBUG: DS4Handler: Emitting filtered gamepad list: {gamepads_info}")
