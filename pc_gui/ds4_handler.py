...
class DS4Handler(threading.Thread):
    ...
    def _refresh_devices(self):
        ...
        print(f"DEBUG: DS4Handler: Found {count} total device(s) that might be joysticks.")

        for i in range(count):
            print(f"INFO: Probing potential joystick at index {i}...")
            try:
                # This is the line that can fail for non-standard devices
                joystick = pygame.joystick.Joystick(i)
                name = joystick.get_name()

                # This is a valid joystick, add it to our clean list
                valid_joysticks.append(joystick)

                # The index we send to the UI is the index in our *new* list of valid joysticks
                new_index = len(valid_joysticks) - 1
                gamepads_info.append({'name': name, 'index': new_index})
                print(f"INFO: Joystick {i} is '{name}'. Added to list at index {new_index}.")

            except pygame.error as e:
                # This is a "bad" joystick (likely the RP2040). Log it and move on.
                print(f"WARN: Could not initialize joystick at index {i}. It will be ignored. Error: {e}")
                continue

        # The main list of joysticks now only contains valid, working devices
        self.joysticks = valid_joysticks

        print(f"DEBUG: DS4Handler: Found {len(self.joysticks)} valid gamepad(s). Emitting list to UI: {gamepads_info}")
