...
class DS4Handler(threading.Thread):
    ...
    def _set_device(self, joystick_index):
        ...
        if joystick_index is not None:
            try:
                # Check if the index is valid for the current list of joysticks
                if joystick_index < len(self.joysticks):
                    self.joystick = self.joysticks[joystick_index]
                    self.joystick.init()
                    print(f"DEBUG: DS4Handler: Successfully initialized joystick: {self.joystick.get_name()}")
                else:
                    # This can happen if a device was unplugged after refresh
                    print(f"ERROR: DS4Handler: Stale joystick index {joystick_index}. Total devices: {len(self.joysticks)}.")
                    self.signals.gamepad_disconnected.emit()
            except pygame.error as e:
                # This catches errors like "Invalid joystick device number" if the OS list changes
                # This can happen in a race condition if a device is unplugged right after a refresh.
                # We handle it gracefully by logging and disconnecting.
                print(f"ERROR: DS4Handler: A pygame error occurred while trying to initialize joystick. This is handled gracefully. Error: {e}")
                self.joystick = None
                self.signals.gamepad_disconnected.emit()
        else:
    ...
    def _refresh_devices(self):
        ...
        This version is simplified to be more stable, avoiding re-initialization of the module.
        """
        print("DEBUG: DS4Handler: Received REFRESH_DEVICES command.")

        # Quit any active joystick instances before clearing the list
        if self.joystick and self.joystick.get_init():
            self.joystick.quit()
            self.joystick = None
        ...
        # We don't need to quit and re-init the whole joystick module.
        # Just get the new count and create new objects.
        ...
        for i in range(count):
            try:
                ...
                # This is where "Couldn't load stick calibration" usually appears.
                # We log it but continue, as it's often a non-fatal issue with a specific device.
                print(f"WARN: DS4Handler: Could not get info for joystick {i}: {e}")
                continue
